...
def train_model(model, train_loader, val_loader=None, num_epochs=10, learning_rate=0.001):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (data, targets) in enumerate(train_loader):
            ...
            # Calculate loss - squeeze to match target shape
            risk_score = outputs['risk_score'].squeeze()
            loss = criterion(risk_score, targets)
            
            # Add pattern detection losses
            for pattern_name in outputs['pattern_scores']:
                # For simplicity, use same targets for all patterns
                pattern_score = outputs['pattern_scores'][pattern_name].squeeze()
                pattern_loss = criterion(pattern_score, targets)
